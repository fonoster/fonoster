...
The `conversationSettings` object allows you to define the conversational behavior of the Autopilot. The conversation settings are independent of the language model used.
...
The `languageModel` object allows you to define the language model used by the Autopilot. The language model is responsible for generating responses to the user's input.
...
Fonoster's Autopilot allows you to use tool to enhance the conversational experience. Tools are used to perform specific actions during the conversation.
...
You can add custom tools to the language model by add an entry to the `tools` array. The following is an example of how to add a custom tool:
