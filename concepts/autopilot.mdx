...
| -------- | ---------------------------------------------------------- | --------------------- |
| OpenAI   | OpenAI provides various GPT models for conversational AI   |                       |
...
<Warning>
  The response from your endpoint must be a JSON with the `result` property as part of it. For example `{ "result": "We have have open slots for Thursday and Friday."}`
</Warning>
...
- : The message sent when the tool is triggered
- : Defines the expected input structure in accordance with the [JSON Schema standard](https://json-schema.org/), which is also required for OpenAI compatible tool calling
  - : Defines the structure of the input (typically `object`)
  - : Specifies the fields expected in the input
