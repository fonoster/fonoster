...
```javascript apply-autopilot-config.js {6}
const fs = require("fs");
const yaml = require("js-yaml");
const SDK = require("@fonoster/sdk");

// Use "fonoster workspaces:active" to get the Workspace Access Key Id
...
Once you create the application, you can use the _updateApplication_ function instead of _createApplication_ to quickly iterate over your Autopilot.
...
| vad.activationThreshold     |                                                                                                                 | 0.4                  |
| vad.deactivationThreshold   |                                                                                                                 | 0.25                 |
| vad.debounceFrames          |                                                                                                                 | 4                    |
...
| Groq      | Groq offers high-performance AI models optimized for speed | `llama-3.3-70b-specdec`, `llama-3.3-70b-versatile`                        |
...
- `parameters`: Defines the expected input structure in accordance with the [JSON Schema standard](https://json-schema.org/), which is also required for OpenAI compatible tool calling
  - `parameters.type`: Defines the structure of the input (typically `object`)
  - `parameters.properties`: Specifies the fields expected in the input
  - `parameters.required`: Lists the fields that must be provided
- `operation`:
  - `operation.method`: The HTTP method (`get` and `post` are supported)
  - `operation.url`: The endpoint to send the request to
  - `operation.headers`: Any necessary headers, such as authentication keys
