...
import { assertsFinishOnKeyIsChar, assertsHasNumDigitsOrTimeout, assertsValuesArePositive } from "../src/gather/asserts";
...
describe("@fonos/voice/gather", () => {
  ...
  context("assert functions", () => {
    it("asserts that options has either a numDigits or a timeout back stop", () => {
      ...
      expect(() => assertsHasNumDigitsOrTimeout({})).to.throw("you must provide either 'numDigits' or 'timeout' option");
    });

    it("ensures that numDigits and timeout are both positive numbers", () => {
      expect(() => assertsValuesArePositive({numDigits: 0})).to.throw("the option 'numDigits' must be a number greater than zero");
      expect(() => assertsValuesArePositive({numDigits: -1})).to.throw("the option 'numDigits' must be a number greater than zero");
      expect(() => assertsValuesArePositive({timeout: 0})).to.not.throw;
      expect(() => assertsValuesArePositive({timeout: -1})).to.throw("the option 'timeout' must be a number equal or greater than zero");
      expect(() => assertsValuesArePositive({timeout: 0, numDigits: 10})).to.not.throw;
    });

    it("finish on key is on the keypad", () => {
      expect(() => assertsFinishOnKeyIsChar({finishOnKey: "-1"})).to.throw("the option 'finishOnKey' must be a single char [0-9], *, or #");
      expect(() => assertsFinishOnKeyIsChar({finishOnKey: "01"})).to.throw("the option 'finishOnKey' must be a single char [0-9], *, or #");
