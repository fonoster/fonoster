...
function updateAgent(agents: AgentsAPI) {
  return withAccess(async (call: { request: UpdateAgentRequest }) => {
    const { ref, name, privacy, domainRef, enabled, maxContacts, expires } =
      call.request;
    ...
    logger.verbose("call to updateAgent", { ref, domainRef });

    return await agents.updateAgent({
      ref,
      name,
      privacy,
      domainRef,
