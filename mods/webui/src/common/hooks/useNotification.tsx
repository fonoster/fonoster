...
import { Snackbar, Alert, AlertTitle, IconButton, Typography, SnackbarOrigin } from '@mui/material';
...
export const useNotification = () => {
  ...
  const notifySuccess = useCallback((message: string, options?: Partial<NotificationOptions>) => {
    notify({
      message,
      type: 'success',
      ...options
  ...
  const notifyWarning = useCallback((message: string, options?: Partial<NotificationOptions>) => {
    notify({
      message,
      type: 'warning',
      ...options
  ...
  const notifyInfo = useCallback((message: string, options?: Partial<NotificationOptions>) => {
    notify({
      message,
      type: 'info',
      ...options
  ...
  const notifySuccessWithStyle = useCallback((message: string, options?: Partial<NotificationOptions>) => {
    notify({
      message,
      type: 'success',
      customStyle: {
  ...
  const NotificationComponent = () => {
    ...
    return (
      <Snackbar
        open={open}
        autoHideDuration={notificationInfo.autoHide ? notificationInfo.duration : null}
        onClose={handleClose}
      ...
      >
        ...
        >
          {notificationInfo.title && <AlertTitle>{notificationInfo.title}</AlertTitle>}
          <Typography variant="body1">
            {notificationInfo.message}
          </Typography>
          {notificationInfo.showCountdown && countdown > 0 && (
            <Typography variant="body2" sx={{ mt: 0.5 }}>
              {countdown > 0 && `You will be redirected in ${countdown} second${countdown !== 1 ? 's' : ''}.`}
            </Typography>
          )}
        </Alert>
      </Snackbar>
