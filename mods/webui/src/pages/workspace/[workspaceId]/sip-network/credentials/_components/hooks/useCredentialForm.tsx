...
export const PASSWORD_REQUIREMENTS = "8+ characters with upper, lower, number, and symbol";
...
export const credentialSchema = z.object({
    ...
    if (data.isEditMode && data.password && !PASSWORD_REGEX.test(data.password)) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: PASSWORD_REQUIREMENTS,
            path: ["password"]
...
export interface CredentialFormOptions {
    ...
    isEditMode: boolean;
    onSuccess?: (credential: { ref: string; name: string; username: string }) => void;
    credentialId?: string;
}
