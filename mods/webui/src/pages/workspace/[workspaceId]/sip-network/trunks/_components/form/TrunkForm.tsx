    ...
    return (
        <PageContainer>
            ...
            <Box sx={{ px: 3, pb: 3 }}>
                ...
                <Box sx={{ mb: 3 }}>
                    <Skeleton variant="rectangular" height={30} width={220} sx={{ borderRadius: 1 }} />
                </Box>
                ...
                {/* Send Register */}
                <Box sx={{ mb: 3 }}>
                    <Skeleton variant="rectangular" height={24} width={150} sx={{ borderRadius: 1 }} />
                </Box>
            </Box>
        </PageContainer>
    );
...
export default function TrunkForm({
    ...
    const [credentials, setCredentials] = useState<{ ref: string; name: string }[]>([]);
    ...
    const [currentCredentialType, setCurrentCredentialType] = useState<'inbound' | 'outbound'>('inbound');
    ...
    const { formState: { isValid }, handleSubmit, setValue } = methods;

    const convertNullToUndefined = (value: string | null | undefined): string | undefined => {
    ...
    const handleCredentialSave = async (credential: CredentialData) => {
        ...
        if (success) {
            const createdCred = credentials.find(c => c.name === credential.username);

            if (createdCred) {
                if (currentCredentialType === 'inbound') {
                    setValue('inboundCredentialsRef', createdCred.ref, { shouldValidate: true });
                } else {
                    setValue('outboundCredentialsRef', createdCred.ref, { shouldValidate: true });
                }
            }
        }
    };
        ...
        <PageContainer>
            ...
            <Box sx={{ mb: 6, mt: -2, maxWidth: DESCRIPTION_MAX_WIDTH }}>
                <Typography variant="body-small">
                    ...
                    regular phones. Complete the following form with the
            ...
            <PageContainer.ContentForm methods={methods} formId={formId}>
                ...
                <Typography variant="body-micro" sx={DESCRIPTION_STYLES}>
                    Outgoing traffic from your communications infrastructure to the PSTN. In order to use a Trunk for
                    termination it must have a Termination SIP URI and at least one authentication scheme
                    (IP Access Control Lists and/or Credential Lists).
                </Typography>
                ...
                <SelectContext
                    ...
                    leadingIcon={null}
                    trailingIcon={isLoadingCredentials ? <CircularProgress size={20} /> : null}
                ...
                <Typography variant="body-micro" sx={DESCRIPTION_STYLES}>
                    Outgoing traffic from your communications infrastructure to the PSTN. In order to use a
                    Trunk for termination it must have a Termination SIP URI and at least one authentication
                    scheme (IP Access Control Lists and/or Credential Lists).
                </Typography>
                ...
                <Typography variant="mono-medium" sx={{ ...SECTION_HEADER_STYLES, mt: 4 }}>
                    OUTBOUND SIP URIS
                </Typography>

                <Typography variant="body-micro" sx={DESCRIPTION_STYLES}>
                    Define the SIP URIs for outbound connections to your provider. You can add multiple URIs
                    with different priorities and weights for load balancing.
                </Typography>
