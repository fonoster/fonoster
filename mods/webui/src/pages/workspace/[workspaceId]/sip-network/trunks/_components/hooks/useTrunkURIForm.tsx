    ...
    host: z.string()
        ...
        .refine(
            (value) => {
                ...
                return ipv4Pattern.test(value) || ipv6Pattern.test(value) || hostnamePattern.test(value);
            },
            { message: "Must be a valid IP address or hostname" }
        ),
    port: z.number().int().min(1, "Port must be a positive number").max(65535, "Port must be less than 65536"),
    transport: z.nativeEnum(Transport),
    user: z.string().optional(),
    weight: z.number().int().min(0, "Weight must be a positive number").default(10),
    priority: z.number().int().min(0, "Priority must be a positive number").default(1),
