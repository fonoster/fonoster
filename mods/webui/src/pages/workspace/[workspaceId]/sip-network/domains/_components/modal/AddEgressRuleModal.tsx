...
export default function AddEgressRuleModal({
    ...
    const { handleSubmit, reset, formState: { isValid, errors }, setValue, watch } = methods;
    ...
    const onSubmit = handleSubmit((data) => {
        const duplicateRule = existingRules.find(rule => rule.numberRef === data.numberRef);

        if (duplicateRule) {
            const numberLabel = outboundNumbers.find(n => n.value === data.numberRef)?.label || data.numberRef;
            setDuplicateError(`A rule already exists for number ${numberLabel} (Rule: ${duplicateRule.rule})`);
            return;
        }
        ...
        >
            <Box sx={{ px: 6, pt: 4 }}>
                <PageContainer>
                    ...
                    <PageContainer.ContentForm methods={methods} formId="egress-rule-form">
                        {duplicateError && (
