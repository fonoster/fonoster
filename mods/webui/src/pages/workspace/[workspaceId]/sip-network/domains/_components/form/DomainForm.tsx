...
export default function DomainForm({
  ...
  useEffect(() => {
    if (
      ...
      egressRulesDisplay.length !== egressRules.length
    ) {
  ...
  useEffect(() => {
    if (initialData?.egressRules && initialData.egressRules.length > 0) {
      ...
      const newEgressRulesMap: Record<
        string,
        { rule: string; numberRef: string }
      > = {};
  ...
  const handleAddEgressRule = (data: { rule: string; numberRef: string }) => {
    ...
    setValue("egressRulesDisplay", newEgressRulesDisplay, {
      shouldValidate: true
    });
  };
