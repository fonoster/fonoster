    ...
    return (
        <PageContainer>
            ...
            <Box sx={{ px: 3, pb: 3 }}>
                ...
                <Box sx={{ mb: 3 }}>
                    <Skeleton variant="rectangular" height={30} width={180} sx={{ borderRadius: 1 }} />
                </Box>
...
export default function AgentForm({
    ...
    const handleCredentialSave = async (credential: { ref: string; name: string; username: string }) => {
        notifySuccess(`Credential "${credential.name}" created and selected successfully`);

        await fetchCredentials();
        methods.setValue('credentialsRef', credential.ref, { shouldValidate: true });
    };
        ...
        <PageContainer>
            ...
            <Box sx={{ mb: 4, mt: -2, maxWidth: DESCRIPTION_MAX_WIDTH }}>
                <Typography variant="body-small">
                    SIP Agents in the same Domain can call each other with Voice
                    Over IP using a Software Phone (e.g Zoiper)
                </Typography>
            </Box>
            ...
            <PageContainer.ContentForm methods={methods} formId={formId}>
                ...
                <SelectContext
                    ...
                    label="Domain URI*"
                    leadingIcon={null}
                    trailingIcon={isLoadingDomains ? <CircularProgress size={20} /> : null}
                    id={`${formId}-domain`}
                    options={domains}
